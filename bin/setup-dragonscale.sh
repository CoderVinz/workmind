...
for required in "scripts/allocate-address.sh" "scripts/tiling-check.py" "skills/wiki-fold/SKILL.md"; do
  if [ ! -e "$required" ]; then
    echo "ERR: missing $required. Reinstall the tablinum plugin." >&2
    exit 1
  fi
done
  ...
  cat > .raw/.manifest.json <<'JSON'
{
  ...
  "description": "Ingest delta tracker and address map for the tablinum vault. Do not hand-edit; wiki-ingest maintains this.",
  "sources": {},
  "address_map": {}
}
JSON
