...
echo "Setting up tablinum vault at: $VAULT"
...
# ── 2. Write graph.json ───────────────────────────────────────────────────────
# Single source of truth: _templates/obsidian/graph.json (12 color groups,
# excludes machinery). Copied here so re-running setup applies graph changes.
cp "$VAULT/_templates/obsidian/graph.json" "$OBSIDIAN/graph.json"
